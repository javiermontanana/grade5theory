


IF we want to be able to write all the notes in a very wide range
let's say the whole range of the piano. 


In treble clef, to write middle C we already need to use a ledger line, 
We could use more and more ledger lines to write lower and lower notes, 
but it quickly becomes hard to read and write these. 


By using a different clef, we can centre the stave on a different set of notes, 
(a different register). This will allow us to write *those* notes without ledger lines, 
and we will need ledger lines for any notes outside that range. 



## G F and C clefs

There are 3 kinds of clefs that we use: 
G clefs, C clefs and F clefs. 

- a G clef looks like this: 
- an F clef looks like this: 
- a C clef looks like this: 
(Try the unicode characters!!!!)


Each of these clefs will be located on a certain staff line, 
and each clef is associated with a particular pitch, 
so the location on the stave of the clef will tell us the position of that pitch 
one the stave. 

- a G clef is associated with the G above middle C. 
- an F clef is associated with the F below middle C.
- a C clef is associated with middle C. 

- The location of the G clef is considered to be the line that the clef starts from. 
- The location of the F clef is where the two dots are centred (also where the clef starts from)
- The location of the C clef is the centre of the clef. 


In theory any clef can be located on any line, 
but in modern use, there are only 4 combinations:

- G clef on the 2nd line (known as treble clef)
- F clef on the 4th line (known as bass clef)
- C clef on the 3rd line (known as alto clef)
- C clef on the 4th line (known as tenor clef)


## 

now that we know what each clef stands for, we can place a given pitch on any clef. 


Let's start by placing middle C in all 4 clefs: 


### In treble clef:

We already know where middle C is but let's use the definition of the clef that we learnt above. 
The G clef on the second line tells us that that is where the note G is, 
and that is the G above middle C, so middle C is a fifth down from it. 
[picture]

### in bass clef: 

The F clef on the 4th line is the F below middle C, 
so middle C is a fifth above: 
[picture]

### in alto and tenor clef: 

The C clef is centred on middle C, 
so in alto clef it will be on the third line, and in tenor clef on the fourth line. 
[picture]



you should memorise where middle C is 



let's place another note 
(F above middle C)




now imagine instead of 5 line staff we have many more. 

then we can place all  3 clefs 


cool visualisation with middle C 
and the other note. 



cool visualisation with a tune. 





















### why so picky about this??






### Grand staff





### 8va and 15a signs 
